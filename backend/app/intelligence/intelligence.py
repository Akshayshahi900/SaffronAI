...
class IntelligenceExtractor:
    ...
    def extract_intel(self, session):
        ...
        full_chat_context = "\n".join(
            f"{m['sender']}: {m['text']}" for m in recent
        )
...
3. Identify detectedLanguage.
4. Identify scamType (OTP Scam, KYC Scam, Bank Impersonation, Refund Scam, Loan Scam, Lottery Scam).
...
"""
        ...
        try:
            ...
            # 🔒 Update intel safely (do NOT overwrite)
            session.intel.update({
            ...
            intel_types = sum([
                bool(session.intel.get("upiIds")),
                bool(session.intel.get("phoneNumbers")),
                bool(session.intel.get("bankAccounts")),
                bool(session.intel.get("phishingLinks")),
            ])

            if turns >= 8 and (intel_types >= 2 or session.intel["confidence"] >= 0.85):
                session.finished = True

            session.agentNotes = (
                f"Language={session.intel['language']} | "
                f"ScamType={session.intel['scamType']}"
            )
...
def infer_attack_flow(session):
    flow = []
    keywords = " ".join(session.intel.get("suspiciousKeywords", [])).lower()
    ...
    if session.intel.get("phoneNumbers"):
        flow.append("Direct Contact")
    if session.intel.get("phishingLinks"):
        flow.append("Phishing Link Delivery")
    if session.intel.get("upiIds") or session.intel.get("bankAccounts"):
...
def calculate_risk(session):
    scam = session.intel.get("scamType", "Unknown")
    base = session.intel.get("confidence", 0.3)
    ...
    urgency = 1 + (len(session.intel.get("suspiciousKeywords", [])) * 0.05)
